# Cranial Nerves III-XII - Ultra-Granular Surgical Anatomy

**Document**: Complete CN III-XII Surgical Anatomy
**Purpose**: Detailed anatomy for neurosurgical procedures
**Scope**: Segments, foramina, function, testing, surgical considerations

---

## CN III - OCULOMOTOR NERVE

```
├─ Type: Motor (to extraocular muscles) + Parasympathetic (pupillary constriction)
│
├─ Origin & Nucleus
│   ├─ Motor nucleus: Midbrain (ventral periaqueductal gray, level of superior colliculus)
│   ├─ Edinger-Westphal nucleus: Parasympathetic (pupil + accommodation)
│   ├─ Fascicles: Traverse through red nucleus and medial cerebral peduncle
│   └─ Exit: Interpeduncular fossa (medial aspect of cerebral peduncle)
│
├─ Segments
│   ├─ 1. Cisternal (Interpeduncular/Crural)
│   │   ├─ Course: Anterolaterally through interpeduncular cistern
│   │   ├─ Length: 24-28 mm
│   │   ├─ Relations:
│   │   │   ├─ Medial: CN III contralateral
│   │   │   ├─ Lateral: Uncus of temporal lobe
│   │   │   ├─ Superior: Posterior cerebral artery (P1-P2 segments)
│   │   │   ├─ Inferior: Superior cerebellar artery (SCA)
│   │   │   └─ Posterior communicating artery (PCoA): Runs lateral/superior to CN III
│   │   │
│   │   ├─ Vascular relationships (CRITICAL):
│   │   │   ├─ PCoA aneurysm: Most common cause of CN III palsy
│   │   │   ├─ Basilar apex aneurysm: May compress CN III
│   │   │   ├─ SCA aneurysm: Rare cause of CN III palsy
│   │   │   └─ Microvascular ischemia: "Medical" CN III palsy (DM, HTN)
│   │   │
│   │   └─ Uncal herniation: Compresses CN III against tentorial edge
│   │       ├─ Pupil dilates ("blown pupil") - parasympathetic fibers compressed
│   │       ├─ Mechanism: Parasympathetic fibers run on surface of nerve
│   │       └─ Sign: Ipsilateral dilated pupil = uncal herniation until proven otherwise!
│   │
│   ├─ 2. Cavernous Sinus Segment
│   │   ├─ Entry: Through dural fold into lateral wall of cavernous sinus
│   │   ├─ Course: Superiormost nerve in lateral wall of CS
│   │   ├─ Length: 20-25 mm
│   │   ├─ Relations in CS:
│   │   │   ├─ Medial: ICA (C4-C6 segments)
│   │   │   ├─ Superior: Anterior clinoid, cavernous ICA roof
│   │   │   ├─ Lateral: Middle cranial fossa dura
│   │   │   ├─ Inferior: CN IV (below), V1 (below), V2 (inferiormost)
│   │   │   └─ CN VI: Within sinus (not in lateral wall)
│   │   │
│   │   └─ Division in CS:
│   │       ├─ Superior division: Superior rectus, levator palpebrae
│   │       └─ Inferior division: Medial rectus, inferior rectus, inferior oblique + parasympathetics
│   │
│   ├─ 3. Orbital Segment
│   │   ├─ Entry: Superior orbital fissure (within annulus of Zinn)
│   │   ├─ Divisions:
│   │   │   ├─ Superior division:
│   │   │   │   ├─ Superior rectus muscle
│   │   │   │   └─ Levator palpebrae superioris (eyelid elevation)
│   │   │   │
│   │   │   └─ Inferior division:
│   │   │       ├─ Medial rectus (eye adduction)
│   │   │       ├─ Inferior rectus (eye depression)
│   │   │       ├─ Inferior oblique (eye elevation + extorsion)
│   │   │       └─ Parasympathetic branch to ciliary ganglion →
│   │   │           ├─ Pupillary sphincter (pupil constriction)
│   │   │           └─ Ciliary muscle (lens accommodation)
│   │   │
│   │   └─ Course: Branches spread to respective muscles
│   │
│   └─ Foramen: Superior orbital fissure (within tendinous ring - annulus of Zinn)
│
├─ Function
│   ├─ Motor:
│   │   ├─ Eye elevation (superior rectus)
│   │   ├─ Eye adduction (medial rectus)
│   │   ├─ Eye depression (inferior rectus)
│   │   ├─ Eye elevation + extorsion (inferior oblique)
│   │   └─ Eyelid elevation (levator palpebrae superioris)
│   │
│   └─ Parasympathetic:
│       ├─ Pupillary constriction (sphincter pupillae)
│       └─ Lens accommodation (ciliary muscle)
│
├─ Testing
│   ├─ Eye movements:
│   │   ├─ "Look up, down, in"
│   │   ├─ H-pattern: Check all directions
│   │   └─ Diplopia (double vision) assessment
│   │
│   ├─ Pupillary light reflex:
│   │   ├─ Direct reflex (light in one eye → constriction same eye)
│   │   ├─ Consensual reflex (light in one eye → constriction other eye)
│   │   └─ Afferent (CN II) vs efferent (CN III) defect
│   │
│   ├─ Ptosis assessment:
│   │   ├─ Eyelid position (levator palpebrae function)
│   │   └─ Complete vs partial ptosis
│   │
│   └─ Accommodation:
│       ├─ Look at distant object, then near object
│       └─ Pupil should constrict with near gaze (accommodation reflex)
│
├─ Clinical Syndromes
│   ├─ Complete CN III Palsy:
│   │   ├─ Eye: Down and out position (lateral rectus + superior oblique unopposed)
│   │   ├─ Ptosis: Complete (levator palpebrae paralysis)
│   │   ├─ Pupil: Dilated and unreactive (parasympathetic damage)
│   │   └─ Diplopia: Severe (cannot adduct, elevate, or depress eye)
│   │
│   ├─ Pupil-Sparing CN III Palsy:
│   │   ├─ Motor deficit present (ptosis, ophthalmoplegia)
│   │   ├─ Pupil NORMAL (reactive, not dilated)
│   │   ├─ Cause: Microvascular ischemia (DM, HTN) - "medical CN III"
│   │   ├─ Mechanism: Core of nerve ischemic, periphery (parasympathetics) spared
│   │   └─ Management: Observation, treat risk factors (usually resolves 3-6 months)
│   │
│   ├─ Pupil-Involving CN III Palsy:
│   │   ├─ Motor deficit + dilated, unreactive pupil
│   │   ├─ Cause: Compression (aneurysm, tumor, herniation)
│   │   ├─ Mechanism: Peripheral nerve fibers (parasympathetics) compressed first
│   │   ├─ EMERGENCY: Assume PCoA aneurysm until proven otherwise!
│   │   └─ Workup: Immediate CTA/MRA, consider angiogram
│   │
│   ├─ Aberrant Regeneration:
│   │   ├─ After CN III injury, axons regrow to wrong muscles
│   │   ├─ Signs:
│   │   │   ├─ Eyelid elevation with downgaze ("pseudo-von Graefe sign")
│   │   │   ├─ Pupil constriction with adduction
│   │   │   └─ Pupil constriction with upgaze
│   │   ├─ Causes: Chronic compression (meningioma, aneurysm, cavernous sinus lesion)
│   │   └─ Never seen with: Microvascular ischemia (ischemia → no regeneration)
│   │
│   └─ Neurological localization:
│       ├─ Nuclear (brainstem): Bilateral ptosis (levator nucleus midline)
│       ├─ Fascicular (midbrain): CN III + other brainstem signs
│       │   ├─ Weber syndrome: CN III + contralateral hemiplegia (cerebral peduncle)
│       │   ├─ Benedikt syndrome: CN III + contralateral tremor (red nucleus)
│       │   └─ Nothnagel syndrome: CN III + cerebellar signs
│       ├─ Cisternal: Isolated CN III, consider aneurysm/tumor
│       ├─ Cavernous: CN III + other CNs (IV, V1, V2, VI)
│       └─ Orbital: CN III + other orbital signs (proptosis, vision changes)
│
└─ Surgical Considerations
    ├─ PCoA aneurysm clipping:
    │   ├─ Most common intraop CN III visualization
    │   ├─ Aneurysm usually lateral to CN III
    │   ├─ Dissection: Identify CN III early, protect throughout
    │   ├─ Temporary clips: Avoid CN III compression
    │   └─ Postop palsy: May be temporary (manipulation) or permanent (injury)
    │
    ├─ Cavernous sinus surgery:
    │   ├─ CN III in superior lateral wall
    │   ├─ Identify and preserve if possible
    │   ├─ Meningiomas: Often cannot preserve (subtotal resection preferred)
    │   └─ Recovery: May take 6-12 months if stretched but not transected
    │
    ├─ Uncal herniation:
    │   ├─ Dilated pupil = CN III compression = herniation
    │   ├─ Treat cause: Evacuate hematoma, osmotic therapy, decompress
    │   └─ Recovery: If nerve not infarcted, pupil may recover over weeks
    │
    └─ Microvascular decompression (rare for CN III):
        ├─ CN III compression by SCA or PCA
        ├─ Approach: Subtemporal, pterional, or orbitozygomatic
        └─ Technique: Interpose Teflon between vessel and nerve
```

---

## CN IV - TROCHLEAR NERVE

```
├─ Type: Motor (to superior oblique muscle only)
│
├─ Unique Features
│   ├─ Only CN to exit brainstem DORSALLY (posterior midbrain)
│   ├─ Only CN to DECUSSATE completely (crosses midline in anterior medullary velum)
│   ├─ Longest intracranial course (~75 mm)
│   ├─ Thinnest CN (smallest diameter)
│   └─ Most vulnerable CN to head trauma (due to long course + thin diameter)
│
├─ Origin & Nucleus
│   ├─ Motor nucleus: Dorsal midbrain (inferior colliculus level)
│   ├─ Fascicles: Dorsal course through midbrain tegmentum
│   ├─ Decussation: Crosses midline in anterior medullary velum (just posterior to inferior colliculus)
│   └─ Exit: Dorsal midbrain (just below inferior colliculus)
│       └─ CRITICAL: Left nucleus → right CN IV, right nucleus → left CN IV
│
├─ Segments
│   ├─ 1. Cisternal (Ambient/Quadrigeminal Cistern)
│   │   ├─ Course: Curves anterolaterally around midbrain (in ambient cistern)
│   │   ├─ Length: 20-25 mm
│   │   ├─ Relations:
│   │   │   ├─ Medial: Midbrain tectum
│   │   │   ├─ Lateral: Temporal lobe (uncus)
│   │   │   ├─ Superior: Tentorium cerebelli (free edge)
│   │   │   ├─ Inferior: Superior cerebellar artery (SCA)
│   │   │   └─ Crosses above: CN III (CN IV more superior)
│   │   │
│   │   ├─ Pierces dura: Just posterior to CN III entry into cavernous sinus
│   │   │
│   │   └─ Vascular relations:
│   │       ├─ SCA loops around midbrain (may compress CN IV)
│   │       ├─ Tentorial branches of ICA/meningohypophyseal trunk
│   │       └─ Rarely compressed by aneurysms (small diameter, high location)
│   │
│   ├─ 2. Cavernous Sinus Segment
│   │   ├─ Entry: Pierces dura lateral to posterior clinoid
│   │   ├─ Course: Lateral wall of cavernous sinus (just inferior to CN III)
│   │   ├─ Length: 20-25 mm
│   │   ├─ Relations in CS:
│   │   │   ├─ Superior: CN III (above)
│   │   │   ├─ Inferior: V1 (ophthalmic division of CN V)
│   │   │   ├─ Medial: ICA
│   │   │   └─ Lateral: Middle cranial fossa dura
│   │   │
│   │   └─ Pathology:
│   │       ├─ CS meningioma: May compress CN IV
│   │       ├─ CS thrombosis: CN IV palsy possible
│   │       └─ Tolosa-Hunt syndrome: Painful ophthalmoplegia (granulomatous inflammation of CS)
│   │
│   ├─ 3. Orbital Segment
│   │   ├─ Entry: Superior orbital fissure (OUTSIDE annulus of Zinn - unlike CN III)
│   │   ├─ Course: Superior to orbit, crosses over superior rectus
│   │   ├─ Target: Superior oblique muscle
│   │   └─ Trochlea: Pulley structure in superomedial orbit (superior oblique tendon loops through)
│   │
│   └─ Foramen: Superior orbital fissure (outside tendinous ring)
│
├─ Function
│   └─ Motor: Superior oblique muscle only
│       ├─ Primary action (eye abducted): Depression
│       ├─ Secondary action: Intorsion (top of eye rotates medially)
│       ├─ Tertiary action: Abduction (minor)
│       └─ Most important for: Downward gaze when eye is adducted (e.g., reading, walking downstairs)
│
├─ Testing
│   ├─ Eye movements:
│   │   ├─ "Look down and in" (best test of superior oblique)
│   │   ├─ H-pattern testing
│   │   └─ Diplopia assessment (vertical or oblique diplopia)
│   │
│   ├─ Head tilt test (Parks-Bielschowsky test):
│   │   ├─ Tilt head to right: Left hypertropia worsens (left CN IV lesion)
│   │   ├─ Tilt head to left: Right hypertropia worsens (right CN IV lesion)
│   │   └─ Mechanism: Unopposed inferior oblique causes hypertropia
│   │
│   └─ Subjective diplopia:
│       ├─ Vertical or oblique (tilted) diplopia
│       ├─ Worse with downgaze (reading, stairs)
│       └─ Compensatory head tilt (away from affected side)
│
├─ Clinical Syndromes
│   ├─ Isolated CN IV Palsy:
│   │   ├─ Hypertropia (vertical deviation): Affected eye higher
│   │   ├─ Excyclotorsion: Top of eye rotated laterally (absent intorsion)
│   │   ├─ Diplopia: Vertical/oblique, worse looking down and in
│   │   ├─ Compensatory: Head tilt to opposite shoulder
│   │   └─ Causes: Trauma (most common), microvascular, tumor, aneurysm (rare)
│   │
│   ├─ Bilateral CN IV Palsy:
│   │   ├─ Often from head trauma (thin nerve, long course, decussation injury)
│   │   ├─ Signs:
│   │   │   ├─ Bilateral hypertropia (alternating)
│   │   │   ├─ Large excyclotorsion (>10°) - best clue to bilateral involvement
│   │   │   ├─ V-pattern esotropia
│   │   │   └─ Difficulty reading (downward gaze impaired)
│   │   └─ Management: Prism glasses, or surgical correction (superior oblique tuck)
│   │
│   ├─ Congenital CN IV Palsy:
│   │   ├─ Often unrecognized until adulthood
│   │   ├─ Clues:
│   │   │   ├─ Longstanding head tilt in old photographs
│   │   │   ├─ Large vertical fusional amplitudes (compensation)
│   │   │   └─ Facial asymmetry (chronic head tilt)
│   │   └─ Treatment: Observation if compensated, or surgery if symptomatic
│   │
│   └─ Neurological localization:
│       ├─ Nuclear/fascicular (midbrain):
│       │   ├─ Crossed palsy: Right nucleus lesion → left CN IV palsy
│       │   └─ Associated signs: Horner's, ataxia, CN III involvement
│       │
│       ├─ Cisternal:
│       │   ├─ Trauma (most common): Shearing injury
│       │   ├─ SCA aneurysm/compression (rare)
│       │   └─ Tentorial meningioma
│       │
│       ├─ Cavernous:
│       │   ├─ CN IV + other CS nerves (III, V1, V2, VI)
│       │   └─ Painful if CS inflammation (Tolosa-Hunt)
│       │
│       └─ Orbital:
│           ├─ CN IV + orbital signs
│           └─ Trauma, tumor, inflammation
│
└─ Surgical Considerations
    ├─ Trauma:
    │   ├─ Most common CN injured in closed head injury
    │   ├─ Long, thin course + tethering at brainstem and SOF = shearing injury
    │   ├─ Often bilateral (decussation injury)
    │   └─ Recovery: May improve over 6-12 months, but often incomplete
    │
    ├─ Microvascular decompression:
    │   ├─ Indication: CN IV compression by SCA
    │   ├─ Approach: Infratentorial supracerebellar (sitting or lateral position)
    │   ├─ Technique: Identify CN IV on superior surface of cerebellum, interpose Teflon
    │   └─ Success rate: 80-90% for vascular compression syndrome
    │
    ├─ Tentorial meningiomas:
    │   ├─ May compress CN IV against tentorium
    │   ├─ Approach: Subtemporal, or supratentorial-infratentorial combined
    │   └─ Prognosis: CN IV may recover if decompressed early
    │
    └─ Surgical palsy (iatrogenic):
        ├─ Can occur during:
        │   ├─ Petrosal approach (CN IV in tentorial edge)
        │   ├─ Resection of tentorial meningioma
        │   └─ Trauma during retraction (thin nerve)
        └─ Prevention: Early identification, gentle handling, avoid excessive retraction
```

---

## CN VI - ABDUCENS NERVE

```
├─ Type: Motor (to lateral rectus muscle only)
│
├─ Unique Features
│   ├─ Longest intracranial course after CN IV
│   ├─ Only nerve running WITHIN cavernous sinus (in blood!)
│   ├─ Most vulnerable CN to increased ICP (false localizing sign)
│   └─ Sharp angle around petrous apex (Dorello's canal)
│
├─ Origin & Nucleus
│   ├─ Motor nucleus: Dorsal pons (pontomedullary junction)
│   │   ├─ Location: Beneath floor of 4th ventricle
│   │   ├─ Facial colliculus: CN VII loops around CN VI nucleus (surface landmark)
│   │   └─ Paramedian pontine reticular formation (PPRF) adjacent
│   │
│   ├─ Exit: Pontomedullary junction (ventral surface)
│   │   └─ Medial to CN VII-VIII complex
│   │
│   └─ Internuclear connections:
│       ├─ MLF (medial longitudinal fasciculus)
│       ├─ Coordinates: CN III (medial rectus) with CN VI (lateral rectus)
│       └─ Horizontal conjugate gaze: Both eyes move together
│
├─ Segments
│   ├─ 1. Cist

ernal (Pontine Cistern to Dorello's Canal)
│   │   ├─ Course: Anterosuperiorly from pontomedullary junction
│   │   ├─ Length: 15-20 mm
│   │   ├─ Relations:
│   │   │   ├─ Basilar artery: Runs lateral to basilar (often compressed by vessel)
│   │   │   ├─ Inferior: Anterior inferior cerebellar artery (AICA)
│   │   │   ├─ Lateral: CN VII-VIII complex, petrosal vein
│   │   │   └─ Ascends: Along clivus toward petrous apex
│   │   │
│   │   ├─ Vulnerability:
│   │   │   ├─ Increased ICP: CN VI has longest cisternal course
│   │   │   ├─ Stretching: Downward brainstem herniation stretches nerve
│   │   │   ├─ "False localizing sign": CN VI palsy doesn't localize lesion
│   │   │   └─ Compression: Mass in posterior fossa, hydrocephalus
│   │   │
│   │   └─ Dorello's Canal Entry
│       │       ├─ Location: Petrous apex (beneath petrosphenoidal ligament - Gruber's ligament)
│       │       ├─ Sharp angle: ~120° turn around petrous apex
│       │       ├─ Fixed: Nerve tethered at Dorello's canal
│       │       └─ Petrous apicitis (Gradenigo syndrome): Inflammation → CN VI palsy
│       │
│   ├─ 2. Dorello's Canal (Petrous Apex)
│   │   ├─ Location: Beneath petrosphenoidal ligament (Gruber's ligament)
│   │   ├─ Length: 5-10 mm
│   │   ├─ Contents: CN VI + inferior petrosal sinus (IPS)
│   │   ├─ Relations:
│   │   │   ├─ Inferior: Inferior petrosal sinus
│   │   │   ├─ Superior: Petrosphenoidal ligament
│   │   │   └─ Lateral: Mastoid air cells, petrous apex bone
│   │   │
│   │   └─ Clinical:
│   │       ├─ Gradenigo syndrome (petrous apicitis):
│   │       │   ├─ Triad: CN VI palsy + facial pain (CN V) + otitis media
│   │       │   ├─ Cause: Infection spreads from middle ear to petrous apex
│   │       │   └─ Treatment: Antibiotics, drainage if abscess
│   │       │
│   │       └─ Petrous apex tumors: CN V + CN VI palsies
│   │
│   ├─ 3. Cavernous Sinus Segment
│   │   ├─ Course: Lateral to ICA, WITHIN sinus (not in lateral wall!)
│   │   ├─ Length: 20-25 mm
│   │   ├─ Location: Medial and inferior (within venous blood)
│   │   │
│   │   ├─ Relations:
│   │   │   ├─ Medial: ICA (immediately lateral to artery)
│   │   │   ├─ Superior: CN III in lateral wall (above)
│   │   │   ├─ Lateral: CN IV, V1, V2 in lateral wall
│   │   │   └─ Surrounded: By venous blood (unique among CNs!)
│   │   │
│   │   ├─ Vulnerability (HIGHEST of all CNs in cavernous sinus):
│   │   │   ├─ Cavernous sinus thrombosis: CN VI affected first (within blood)
│   │   │   ├─ Mass effect: Any CS lesion compresses CN VI early
│   │   │   ├─ Carotid-cavernous fistula: Venous hypertension → CN VI palsy
│   │   │   └─ Pituitary apoplexy: Lateral extension → CN VI compression
│   │   │
│   │   └─ Pathology affecting CN VI in CS:
│   │       ├─ Cavernous sinus meningioma
│   │       ├─ Carotid aneurysm (cavernous ICA)
│   │       ├─ Nasopharyngeal carcinoma (perineural spread)
│   │       └─ Tolosa-Hunt syndrome (painful ophthalmoplegia)
│   │
│   └─ 4. Orbital Segment
│       ├─ Entry: Superior orbital fissure (within annulus of Zinn)
│       ├─ Course: Directly to lateral rectus muscle (medial surface)
│       ├─ Target: Lateral rectus muscle (sole innervation)
│       └─ No branches (simplest motor nerve - single muscle)
│
├─ Foramen: Superior orbital fissure (within tendinous ring - annulus of Zinn)
│
├─ Function
│   └─ Motor: Lateral rectus muscle only
│       ├─ Eye abduction (turn eye outward, away from nose)
│       └─ Horizontal gaze (with CN III medial rectus for conjugate gaze)
│
├─ Testing
│   ├─ Eye movements: "Look to the right, look to the left"
│   ├─ H-pattern testing
│   ├─ Diplopia assessment: Horizontal diplopia (worse looking to affected side)
│   └─ Abduction failure: Eye cannot move laterally past midline
│
├─ Clinical Syndromes
│   ├─ Complete CN VI Palsy:
│   │   ├─ Eye position: Esotropia (eye deviated medially - adducted)
│   │   ├─ Unopposed: Medial rectus (CN III) pulls eye inward
│   │   ├─ Abduction failure: Cannot move eye laterally
│   │   ├─ Diplopia: Horizontal, worse in far gaze to affected side
│   │   └─ Head turn: To affected side (compensatory - reduces diplopia)
│   │
│   ├─ Causes by Location:
│   │   ├─ Nuclear/fascicular (pons):
│   │   │   ├─ Stroke: Pontine infarct (often with hemiparesis)
│   │   │   ├─ Demyelination: MS plaque
│   │   │   ├─ Wernicke's encephalopathy: Thiamine deficiency
│   │   │   └─ Associated: PPRF involvement → horizontal gaze palsy (not just abduction)
│   │   │
│   │   ├─ Cisternal:
│   │   │   ├─ Increased ICP: Most common (false localizing sign)
│   │   │   │   └─ Mechanism: Downward pressure → stretching over petrous apex
│   │   │   ├─ Meningitis: Bacterial, carcinomatous
│   │   │   ├─ Trauma: Skull base fracture, petrous fracture
│   │   │   └─ Chiari malformation: Brainstem compression
│   │   │
│   │   ├─ Dorello's canal:
│   │   │   ├─ Gradenigo syndrome (petrous apicitis): CN VI + CN V pain + otitis
│   │   │   ├─ Petrous apex tumor: Chondrosarcoma, chordoma
│   │   │   └─ Petrous fracture: Trauma
│   │   │
│   │   ├─ Cavernous sinus:
│   │   │   ├─ Cavernous sinus thrombosis: CN VI first, then III, IV, V
│   │   │   ├─ Carotid-cavernous fistula: Pulsatile proptosis, bruit
│   │   │   ├─ Cavernous sinus meningioma
│   │   │   ├─ Pituitary apoplexy: Sudden headache, vision loss, CN VI palsy
│   │   │   └─ Nasopharyngeal carcinoma: Perineural spread
│   │   │
│   │   └─ Orbital:
│   │       ├─ Orbital tumor
│   │       ├─ Orbital trauma
│   │       └─ Thyroid eye disease (rare - usually mechanical restriction)
│   │
│   ├─ Bilateral CN VI Palsy:
│   │   ├─ Causes: Increased ICP, basilar tip aneurysm, pontine lesion
│   │   ├─ Presentation: Bilateral esotropia, cannot abduct either eye
│   │   └─ Diplopia: Horizontal, worse in far gaze (both directions)
│   │
│   ├─ Microvascular (Vasculopathic) CN VI Palsy:
│   │   ├─ Risk factors: Diabetes, hypertension, age >50
│   │   ├─ Mechanism: Ischemia to nerve (vasa nervorum occlusion)
│   │   ├─ Presentation: Sudden onset, isolated CN VI palsy, no other deficits
│   │   ├─ Recovery: Usually spontaneous over 3-6 months
│   │   └─ Workup: If typical (age, risk factors), observe; if atypical → MRI
│   │
│   └─ Duane Retraction Syndrome:
│       ├─ Congenital absence of CN VI nucleus
│       ├─ CN III aberrantly innervates lateral rectus
│       ├─ Presentation: Limited abduction + globe retraction on adduction
│       └─ Not a true CN VI palsy (developmental anomaly)
│
└─ Surgical Considerations
    ├─ Petrous apex surgery:
    │   ├─ Dorello's canal: Identify and protect CN VI
    │   ├─ Petrous apicectomy: Risk CN VI injury
    │   └─ Recovery: Variable if nerve stretched but not transected
    │
    ├─ Cavernous sinus surgery:
    │   ├─ CN VI most vulnerable (within blood, adjacent to ICA)
    │   ├─ Often cannot preserve if extensive tumor
    │   └─ Postop CN VI palsy: Common, may improve over 6-12 months
    │
    ├─ Clivus/skull base surgery:
    │   ├─ Transclival approach: CN VI at risk
    │   └─ Midline lesions: Can have bilateral CN VI injury
    │
    └─ Management of CN VI palsy:
        ├─ Acute: Patch eye or prism glasses (reduce diplopia)
        ├─ Subacute: Observe 6 months (may recover)
        ├─ Chronic (>6 mo, no recovery): Strabismus surgery
        │   ├─ Recession-resection: Weaken medial rectus, strengthen lateral rectus
        │   └─ Transposition procedures: Vertical recti to assist abduction
        └─ Botulinum toxin: Temporary weakening of medial rectus
```

---

*CN V (Trigeminal) was documented separately due to its extensive complexity (3 divisions, ~10,000 words). See CN_V_TRIGEMINAL_COMPLETE.md for complete V1, V2, V3 anatomy, autonomic ganglia, muscles of mastication, and trigeminal neuralgia management.*

---

## CN VII - FACIAL NERVE

```
├─ Type: Mixed - Motor (major) + Sensory + Parasympathetic
│
├─ Components (Complex!)
│   ├─ Motor (Branchial motor - special visceral efferent):
│   │   └─ Muscles of facial expression (all)
│   │
│   ├─ Sensory (Special sensory - taste):
│   │   └─ Taste from anterior 2/3 tongue (via chorda tympani)
│   │
│   ├─ Parasympathetic (General visceral efferent):
│   │   ├─ Lacrimation (lacrimal gland - via greater petrosal nerve)
│   │   ├─ Salivation (submandibular/sublingual glands - via chorda tympani)
│   │   └─ Nasal/palatine glands (via greater petrosal nerve → pterygopalatine ganglion)
│   │
│   └─ Sensory (General sensory):
│       └─ Small area external ear (concha, EAC)
│
├─ Origin & Nuclei
│   ├─ Motor nucleus:
│   │   ├─ Location: Ventrolateral pons (pontine tegmentum)
│   │   ├─ Organization:
│   │   │   ├─ Upper face (forehead): BILATERAL cortical input
│   │   │   └─ Lower face (below eyes): CONTRALATERAL cortical input only
│   │   └─ Clinical: UMN lesion spares forehead, LMN lesion affects entire face
│   │
│   ├─ Superior salivatory nucleus:
│   │   ├─ Parasympathetic preganglionic neurons
│   │   └─ Location: Pontine tegmentum (rostral to inferior salivatory nucleus)
│   │
│   ├─ Nucleus of solitary tract (gustatory portion):
│   │   ├─ Taste information
│   │   └─ Rostral portion (anterior to CN IX/X taste input)
│   │
│   └─ Spinal trigeminal nucleus:
│       └─ General sensory from ear (small contribution)
│
├─ Segments (7 Segments - Complex Course!)
│   │
│   ├─ 1. Cisternal (Pontomedullary Junction to IAC)
│   │   ├─ Origin: Pontomedullary junction (cerebellopontine angle)
│   │   ├─ Two roots emerge:
│   │   │   ├─ Motor root (larger, lateral)
│   │   │   └─ Nervus intermedius (Wrisberg) - sensory/parasympathetic (smaller, medial)
│   │   │
│   │   ├─ Course: Laterally across CPA cistern with CN VIII
│   │   ├─ Length: 15-24 mm
│   │   │
│   │   ├─ Relations:
│   │   │   ├─ CN VIII: Immediately adjacent (CN VII superior-anterior, CN VIII inferior-posterior)
│   │   │   ├─ AICA: Loops in CPA, may compress nerves (hemifacial spasm, tinnitus)
│   │   │   ├─ Flocculus (cerebellum): Posteroinferior
│   │   │   └─ Pons: Medial
│   │   │
│   │   ├─ Entry: Internal auditory canal (IAC) - porus acusticus
│   │   │
│   │   └─ Clinical:
│   │       ├─ CPA tumors: Acoustic neuroma (most common), meningioma, epidermoid
│   │       ├─ CN VII + CN VIII dysfunction: Hearing loss, facial weakness, tinnitus, vertigo
│   │       └─ Hemifacial spasm: AICA compression of CN VII at REZ → involuntary facial contractions
│   │
│   ├─ 2. Meatal (Internal Auditory Canal)
│   │   ├─ Course: Lateral through IAC with CN VIII
│   │   ├─ Length: 8-10 mm
│   │   ├─ Organization within IAC:
│   │   │   ├─ Anterior-superior: CN VII
│   │   │   ├─ Anterior-inferior: Cochlear nerve
│   │   │   ├─ Posterior-superior: Superior vestibular nerve
│   │   │   └─ Posterior-inferior: Inferior vestibular nerve
│   │   │
│   │   ├─ Labyrinthine artery: Travels with nerves in IAC
│   │   │
│   │   └─ Clinical:
│   │       ├─ Acoustic neuroma: Originates from CN VIII but compresses CN VII
│   │       ├─ Surgical exposure: Translabyrinthine or retrosigmoid approach
│   │       └─ Hearing preservation: Depends on approach and tumor size
│   │
│   ├─ 3. Labyrinthine (Narrowest Segment!)
│   │   ├─ Course: IAC fundus → geniculate ganglion
│   │   ├─ Length: 3-5 mm
│   │   ├─ Diameter: 0.61 mm (NARROWEST part of entire nerve!)
│   │   │
│   │   ├─ Relations:
│   │   │   ├─ Cochlea: Anterior and inferior
│   │   │   ├─ Vestibule: Posterior
│   │   │   └─ Bone: Surrounded by fallopian canal (bony canal)
│   │   │
│   │   └─ Clinical (CRITICAL!):
│   │       ├─ Bell's palsy: Viral inflammation (HSV-1) → swelling → compression in narrow canal
│   │       ├─ "Bottleneck effect": Swelling here causes ischemia → facial paralysis
│   │       ├─ Temporal bone fracture: Labyrinthine segment most vulnerable
│   │       └─ Decompression surgery: Opens bone around this segment
│   │
│   ├─ 4. Geniculate Ganglion (NOT a segment, but key landmark)
│   │   ├─ Location: Sharp anterior bend ("genu" = knee)
│   │   ├─ Contains: Sensory neuron cell bodies (for taste - geniculate ganglion)
│   │   ├─ Branch: Greater petrosal nerve exits here
│   │   │
│   │   └─ Greater Petrosal Nerve (Parasympathetic Branch)
│   │       ├─ Course: Anteromedially from geniculate ganglion
│   │       ├─ Hiatus: Exits via hiatus of facial canal (middle cranial fossa)
│   │       ├─ Joins: Deep petrosal nerve (sympathetics from ICA plexus) → forms Vidian nerve
│   │       ├─ Destination: Pterygopalatine ganglion (via pterygoid canal)
│   │       ├─ Synapse: Preganglionic CN VII → postganglionic at pterygopalatine ganglion
│   │       │
│   │       └─ Function:
│   │           ├─ Lacrimation: Lacrimal gland (via V2 zygomatic → V1 lacrimal)
│   │           ├─ Nasal secretion: Nasal glands
│   │           └─ Palatine secretion: Palatine glands
│   │
│   ├─ 5. Tympanic (Horizontal Segment)
│   │   ├─ Course: Posterolaterally along medial wall of middle ear
│   │   ├─ Length: 11-13 mm
│   │   │
│   │   ├─ Relations:
│   │   │   ├─ Superior: Lateral semicircular canal
│   │   │   ├─ Inferior: Oval window, stapes footplate
│   │   │   ├─ Anterior: Cochlea (cochleariform process)
│   │   │   └─ Medial: Bone (fallopian canal)
│   │   │
│   │   ├─ Branch: Nerve to stapedius
│   │   │   ├─ Exits: From tympanic segment
│   │   │   ├─ Target: Stapedius muscle
│   │   │   └─ Function: Stapedius reflex (dampens loud sounds, protects inner ear)
│   │   │
│   │   └─ Clinical:
│   │       ├─ Chronic otitis media: Can erode fallopian canal → facial weakness
│   │       ├─ Cholesteatoma: Erosion of bone → CN VII exposure/injury
│   │       ├─ Middle ear surgery: CN VII at risk (identify before drilling)
│   │       └─ Hyperacusis: If nerve to stapedius injured (sounds abnormally loud)
│   │
│   ├─ 6. Mastoid (Vertical/Descending Segment)
│   │   ├─ Course: Descends vertically in mastoid portion of temporal bone
│   │   ├─ Length: 10-14 mm
│   │   │
│   │   ├─ Relations:
│   │   │   ├─ Lateral: Mastoid air cells
│   │   │   ├─ Medial: Medial wall of tympanic cavity
│   │   │   ├─ Posterior: Tympanic sinus
│   │   │   └─ Anterior: Tympanic membrane (posterior)
│   │   │
│   │   ├─ Branch: Chorda Tympani (Parasympathetic + Taste)
│   │   │   ├─ Origin: Exits CN VII ~6 mm above stylomastoid foramen
│   │   │   ├─ Course: Anterosuperiorly → enters middle ear cavity
│   │   │   │   └─ Crosses: Medial surface of tympanic membrane (behind eardrum)
│   │   │   │   └─ Between: Handle of malleus and long process of incus
│   │   │   │
│   │   │   ├─ Exit middle ear: Petrotympanic fissure (Glaserian fissure)
│   │   │   ├─ Joins: Lingual nerve (CN V3) in infratemporal fossa
│   │   │   │
│   │   │   ├─ Components:
│   │   │   │   ├─ Taste: Anterior 2/3 of tongue (special sensory)
│   │   │   │   └─ Parasympathetic: Submandibular + sublingual glands (salivation)
│   │   │   │
│   │   │   ├─ Pathway for salivation:
│   │   │   │   ├─ Preganglionic: CN VII → chorda tympani → lingual nerve
│   │   │   │   ├─ Synapse: Submandibular ganglion
│   │   │   │   └─ Postganglionic: To submandibular + sublingual glands
│   │   │   │
│   │   │   └─ Clinical:
│   │   │       ├─ Middle ear surgery: Chorda tympani often sacrificed
│   │   │       ├─ Results: Loss of taste anterior 2/3 tongue + decreased salivation
│   │   │       ├─ Tympanoplasty: Risk chorda tympani injury
│   │   │       └─ Metallic taste: Regeneration phenomenon
│   │   │
│   │   └─ Other branches:
│   │       └─ Posterior auricular nerve (exits near stylomastoid foramen)
│   │           ├─ Motor: Occipitalis muscle, posterior auricular muscle
│   │           └─ Sensory: Small area behind ear
│   │
│   └─ 7. Extratemporal (After Stylomastoid Foramen)
│       ├─ Exit: Stylomastoid foramen (between styloid and mastoid processes)
│       │
│       ├─ Branches immediately after exit:
│       │   ├─ Posterior auricular nerve (see above)
│       │   ├─ Nerve to posterior belly of digastric
│       │   └─ Nerve to stylohyoid muscle
│       │
│       ├─ Entry into parotid gland:
│       │   ├─ Passes lateral to styloid process
│       │   ├─ Enters: Posterior surface of parotid gland
│       │   ├─ Superficial to: Retromandibular vein, external carotid artery
│       │   └─ Divides: Within parotid into terminal branches
│       │
│       └─ Terminal Branches (5-6 major branches)
│           ├─ Temporal (Frontal) Branch:
│           │   ├─ Course: Superiorly across zygomatic arch
│           │   ├─ Motor:
│           │   │   ├─ Frontalis (forehead elevation, eyebrow raise)
│           │   │   ├─ Orbicularis oculi (superior portion - eye closure)
│           │   │   └─ Corrugator supercilii (eyebrow depression - frowning)
│           │   │
│           │   └─ Clinical: Injury → inability to raise eyebrow, incomplete eye closure (lagophthalmos risk)
│           │
│           ├─ Zygomatic Branch:
│           │   ├─ Course: Across zygoma
│           │   ├─ Motor:
│           │   │   ├─ Orbicularis oculi (lateral portion - eye closure)
│           │   │   └─ Muscles of lateral face
│           │   │
│           │   └─ Anastomoses with temporal branch (protective redundancy)
│           │
│           ├─ Buccal Branch(es):
│           │   ├─ Usually 2-3 branches
│           │   ├─ Motor:
│           │   │   ├─ Buccinator (cheek compression - chewing, whistling)
│           │   │   ├─ Orbicularis oris (upper lip - lip closure)
│           │   │   ├─ Levator anguli oris (smile - elevate angle of mouth)
│           │   │   └─ Zygomaticus major/minor (smile - elevate upper lip)
│           │   │
│           │   └─ Multiple branches: Good anastomoses (protective)
│           │
│           ├─ Marginal Mandibular Branch:
│           │   ├─ Course: Along inferior border of mandible
│           │   ├─ Relationship to mandible:
│           │   │   ├─ Runs 1-2 cm BELOW inferior border (usually)
│           │   │   ├─ Can be up to 4 cm below in some people
│           │   │   └─ At risk during submandibular gland surgery, neck dissection
│           │   │
│           │   ├─ Motor:
│           │   │   ├─ Depressor anguli oris (depress corner of mouth - frown)
│           │   │   ├─ Depressor labii inferioris (depress lower lip)
│           │   │   ├─ Mentalis (chin elevation - pouting)
│           │   │   └─ Orbicularis oris (lower lip)
│           │   │
│           │   └─ Clinical:
│           │       ├─ Injury: Asymmetric smile (drooping corner on affected side)
│           │       ├─ Preservation: Key goal in parotidectomy, neck surgery
│           │       └─ Identification: Intraoperative nerve monitoring
│           │
│           └─ Cervical Branch:
│               ├─ Course: Inferiorly into neck
│               ├─ Motor: Platysma (neck skin movement)
│               └─ Clinical: Least clinically important branch
│
├─ Testing Methods (Comprehensive Assessment)
│   │
│   ├─ Motor Function Testing:
│   │   ├─ Forehead:
│   │   │   ├─ "Raise your eyebrows" → Frontalis function
│   │   │   ├─ "Wrinkle your forehead" → Frontalis contraction
│   │   │   └─ Asymmetry indicates LMN lesion (UMN spares forehead!)
│   │   │
│   │   ├─ Eye closure:
│   │   │   ├─ "Close your eyes tightly" → Orbicularis oculi
│   │   │   ├─ Attempt to open against resistance
│   │   │   ├─ Incomplete closure: Bell's phenomenon (eye rolls up)
│   │   │   └─ Lagophthalmos: Inability to close = corneal exposure risk!
│   │   │
│   │   ├─ Smile:
│   │   │   ├─ "Show me your teeth" → Zygomaticus, levator anguli oris
│   │   │   ├─ Asymmetry: Affected side droops, nasolabial fold flattened
│   │   │   └─ Corner of mouth: Affected side cannot elevate
│   │   │
│   │   ├─ Puff cheeks:
│   │   │   ├─ "Puff out your cheeks" → Buccinator, orbicularis oris
│   │   │   ├─ Air leaks: Lip weakness, cannot seal
│   │   │   └─ Test: Press on cheeks, assess seal integrity
│   │   │
│   │   ├─ Lower face:
│   │   │   ├─ "Show me your bottom teeth" → Depressor labii inferioris
│   │   │   ├─ "Pout your lip" → Mentalis
│   │   │   └─ Asymmetric lower lip: Marginal mandibular branch injury
│   │   │
│   │   └─ Platysma:
│   │       ├─ "Grimace and tense your neck" → Platysma contraction
│   │       └─ Rarely tested clinically
│   │
│   ├─ UMN vs LMN Distinction (CRITICAL!)
│   │   │
│   │   ├─ Upper Motor Neuron (UMN) Lesion:
│   │   │   ├─ Location: Cortex, internal capsule, brainstem (above CN VII nucleus)
│   │   │   ├─ Forehead: SPARED (bilateral cortical input to upper face)
│   │   │   ├─ Lower face: WEAK (contralateral cortical input only)
│   │   │   │
│   │   │   ├─ Presentation:
│   │   │   │   ├─ Can raise eyebrows (forehead spared)
│   │   │   │   ├─ Can wrinkle forehead (frontalis works)
│   │   │   │   ├─ Can close eyes (orbicularis oculi works)
│   │   │   │   ├─ Cannot smile (lower face weak)
│   │   │   │   └─ Flattened nasolabial fold
│   │   │   │
│   │   │   ├─ Causes:
│   │   │   │   ├─ Stroke (most common): Middle cerebral artery territory
│   │   │   │   ├─ Brainstem lesion (if below decussation)
│   │   │   │   ├─ Tumor: Frontal or parietal lobe
│   │   │   │   └─ Demyelination: MS, other
│   │   │   │
│   │   │   └─ Associated signs:
│   │   │       ├─ Contralateral hemiparesis (arm/leg weakness)
│   │   │       ├─ Dysarthria (if cortical/subcortical)
│   │   │       └─ Other cortical signs (aphasia, neglect)
│   │   │
│   │   └─ Lower Motor Neuron (LMN) Lesion:
│   │       ├─ Location: CN VII nucleus, nerve (anywhere along course)
│   │       ├─ Forehead: WEAK (no bilateral compensation)
│   │       ├─ Lower face: WEAK
│   │       │
│   │       ├─ Presentation:
│   │       │   ├─ Cannot raise eyebrows (frontalis paralyzed)
│   │       │   ├─ Cannot wrinkle forehead
│   │       │   ├─ Cannot close eye (orbicularis oculi paralyzed)
│   │       │   ├─ Bell's phenomenon: Eye rolls up on attempted closure
│   │       │   ├─ Cannot smile (entire face affected)
│   │       │   └─ Drooling (orbicularis oris weak)
│   │       │
│   │       ├─ Causes: Bell's palsy, Ramsay Hunt, tumor, trauma, stroke (pontine)
│   │       │
│   │       └─ Localization by branch involvement:
│   │           ├─ Proximal to greater petrosal: Dry eye + facial weakness
│   │           ├─ Distal to greater petrosal, proximal to stapedius: Hyperacusis + facial weakness
│   │           ├─ Distal to stapedius, proximal to chorda tympani: Taste loss + facial weakness
│   │           └─ Distal to chorda tympani: Isolated facial weakness only
│   │
│   ├─ Taste Testing:
│   │   ├─ Anterior 2/3 tongue: CN VII (via chorda tympani)
│   │   ├─ Test: Sweet, salty, sour, bitter on anterior tongue
│   │   ├─ Loss: Lesion proximal to chorda tympani branch
│   │   └─ Metallic taste: Chorda tympani irritation/regeneration
│   │
│   ├─ Lacrimation (Schirmer Test):
│   │   ├─ Test: Filter paper in lower eyelid, measure tear production
│   │   ├─ Normal: >10 mm wetting in 5 minutes
│   │   ├─ Decreased: Lesion proximal to greater petrosal nerve
│   │   └─ Paradoxical lacrimation: "Crocodile tears" during eating (aberrant regeneration)
│   │
│   ├─ Stapedius Reflex:
│   │   ├─ Test: Acoustic reflex testing (loud sound → stapedius contracts)
│   │   ├─ Absent: Lesion proximal to nerve to stapedius
│   │   └─ Hyperacusis: Stapedius paralysis → sounds abnormally loud
│   │
│   └─ Blink Reflex (Corneal Reflex):
│       ├─ Afferent: CN V (trigeminal - corneal sensation)
│       ├─ Efferent: CN VII (facial - eye closure)
│       ├─ Test: Touch cornea → both eyes blink
│       └─ Abnormal: CN VII lesion → ipsilateral eye doesn't blink
│
├─ Clinical Syndromes (Major)
│   │
│   ├─ Bell's Palsy (Idiopathic Facial Paralysis)
│   │   │
│   │   ├─ Definition: Acute onset unilateral LMN facial paralysis (no identifiable cause)
│   │   ├─ Incidence: 20-30 per 100,000 per year
│   │   ├─ Age: All ages, peak 15-45 years
│   │   ├─ Risk factors: Pregnancy (3x risk, especially 3rd trimester), diabetes
│   │   │
│   │   ├─ Etiology (Current Understanding):
│   │   │   ├─ Viral hypothesis (most accepted):
│   │   │   │   ├─ HSV-1 (Herpes simplex): Most common (~70%)
│   │   │   │   ├─ VZV (Varicella zoster): If Ramsay Hunt (see below)
│   │   │   │   ├─ EBV, CMV, adenovirus (less common)
│   │   │   │   └─ Mechanism: Viral reactivation → inflammation → edema
│   │   │   │
│   │   │   ├─ Pathophysiology:
│   │   │   │   ├─ Viral inflammation → nerve swelling
│   │   │   │   ├─ Labyrinthine segment: Narrowest (0.61 mm) → compression
│   │   │   │   ├─ "Bottleneck effect": Swelling → ischemia → demyelination
│   │   │   │   ├─ Ischemia → conduction block → paralysis
│   │   │   │   └─ Severe: Axonal degeneration (poor prognosis)
│   │   │   │
│   │   │   └─ Vascular hypothesis (alternative):
│   │   │       └─ Microvascular ischemia → nerve dysfunction
│   │   │
│   │   ├─ Presentation:
│   │   │   ├─ Onset: Acute (hours to days), often overnight
│   │   │   ├─ Prodrome (50%): Retroauricular pain 1-2 days before weakness
│   │   │   │
│   │   │   ├─ Motor:
│   │   │   │   ├─ Complete unilateral facial paralysis (forehead + lower face)
│   │   │   │   ├─ Cannot raise eyebrow, cannot close eye, cannot smile
│   │   │   │   ├─ Bell's phenomenon: Eye rolls up on attempted closure
│   │   │   │   └─ Drooling, food collects in cheek
│   │   │   │
│   │   │   ├─ Sensory:
│   │   │   │   ├─ Taste loss: Anterior 2/3 tongue (if chorda tympani involved)
│   │   │   │   ├─ Hyperacusis: Sounds loud (stapedius paralysis)
│   │   │   │   └─ Decreased lacrimation: Dry eye (if greater petrosal involved)
│   │   │   │
│   │   │   ├─ Eye complications (URGENT!):
│   │   │   │   ├─ Lagophthalmos: Cannot close eye → corneal exposure
│   │   │   │   ├─ Dry eye: Decreased blinking, decreased tear production
│   │   │   │   ├─ Corneal abrasion/ulceration: Can lead to blindness
│   │   │   │   └─ Requires: Aggressive eye protection (artificial tears, taping, moisture chamber)
│   │   │   │
│   │   │   └─ Diagnosis of exclusion: Must rule out other causes!
│   │   │       ├─ Stroke (UMN): Forehead spared
│   │   │       ├─ Ramsay Hunt syndrome: Vesicles in ear
│   │   │       ├─ Lyme disease: History of tick exposure, erythema migrans
│   │   │       ├─ Tumor: Gradual onset, other CN involvement
│   │   │       └─ Trauma: History of injury
│   │   │
│   │   ├─ Workup:
│   │   │   ├─ Clinical diagnosis: History + exam sufficient
│   │   │   ├─ MRI: NOT routine, only if atypical (gradual onset, no recovery, other CNs)
│   │   │   ├─ Lyme serology: If endemic area, summer, tick exposure
│   │   │   ├─ Diabetes screening: Check glucose
│   │   │   └─ EMG/NCS: If considering surgical decompression (rarely done)
│   │   │
│   │   ├─ Treatment (Acute Phase - First 72 Hours):
│   │   │   ├─ Corticosteroids (Grade A Evidence):
│   │   │   │   ├─ Prednisolone: 60-80 mg/day x 7 days, then taper
│   │   │   │   ├─ Start: Within 72 hours (earlier better)
│   │   │   │   ├─ Mechanism: Reduce inflammation, edema, compression
│   │   │   │   ├─ Benefit: Increases recovery rate from ~70% to ~94%
│   │   │   │   └─ Contraindications: Immunosuppression, uncontrolled DM
│   │   │   │
│   │   │   ├─ Antivirals (Controversial):
│   │   │   │   ├─ Acyclovir 400 mg 5x/day OR Valacyclovir 1000 mg 3x/day x 7 days
│   │   │   │   ├─ Evidence: Mixed (some benefit if combined with steroids)
│   │   │   │   ├─ Cochrane review: Minimal benefit
│   │   │   │   └─ Current: Often added to steroids (low harm)
│   │   │   │
│   │   │   └─ Eye Protection (CRITICAL!):
│   │   │       ├─ Artificial tears: Every 1-2 hours while awake
│   │   │       ├─ Lubricating ointment: At night
│   │   │       ├─ Tape eye shut: At night (if cannot close)
│   │   │       ├─ Moisture chamber: Protective glasses/goggles
│   │   │       └─ Ophthalmology referral: If corneal exposure severe
│   │   │
│   │   ├─ Prognosis:
│   │   │   ├─ Complete recovery: 70-80% (within 3-6 months)
│   │   │   ├─ Partial recovery: 15-20%
│   │   │   ├─ No recovery: 5% (poor prognostic signs below)
│   │   │   │
│   │   │   ├─ Good prognosis:
│   │   │   │   ├─ Incomplete paralysis at onset
│   │   │   │   ├─ Early recovery (within 3 weeks)
│   │   │   │   ├─ Young age
│   │   │   │   └─ No hyperacusis (nerve to stapedius spared)
│   │   │   │
│   │   │   └─ Poor prognosis:
│   │   │       ├─ Complete paralysis at onset
│   │   │       ├─ No recovery by 3-4 months
│   │   │       ├─ Elderly
│   │   │       ├─ Diabetes
│   │   │       └─ Severe pain, hyperacusis (proximal nerve involvement)
│   │   │
│   │   ├─ Sequelae (Chronic):
│   │   │   ├─ Synkinesis (aberrant regeneration):
│   │   │   │   ├─ Mechanism: Axons regenerate to wrong muscles
│   │   │   │   ├─ Example: Eye closure → corner of mouth elevates
│   │   │   │   ├─ Example: Smiling → eye closure
│   │   │   │   └─ Treatment: Botulinum toxin (BoNT) to affected muscles
│   │   │   │
│   │   │   ├─ Crocodile tears (gustatolacrimal reflex):
│   │   │   │   ├─ Mechanism: Salivatory fibers regenerate to lacrimal gland
│   │   │   │   ├─ Presentation: Tearing when eating
│   │   │   │   └─ Treatment: BoNT to lacrimal gland
│   │   │   │
│   │   │   ├─ Contracture:
│   │   │   │   ├─ Facial muscles tonically contracted
│   │   │   │   ├─ Appearance: Tighter on affected side
│   │   │   │   └─ Treatment: BoNT, physical therapy
│   │   │   │
│   │   │   └─ Residual weakness:
│   │   │       ├─ Subtle asymmetry with movement
│   │   │       └─ May benefit from facial reanimation (if severe)
│   │   │
│   │   └─ Surgical Decompression (RARE, Controversial):
│   │       ├─ Indication: Complete paralysis + no recovery + EMG shows severe degeneration
│   │       ├─ Timing: Within 2 weeks of onset
│   │       ├─ Technique: Transmastoid decompression of labyrinthine segment
│   │       ├─ Evidence: Limited, not standard of care
│   │       └─ Current use: Rare, only specialized centers
│   │
│   ├─ Ramsay Hunt Syndrome (Herpes Zoster Oticus)
│   │   ├─ Definition: VZV (varicella zoster) reactivation in geniculate ganglion
│   │   ├─ Incidence: 5 per 100,000 per year
│   │   │
│   │   ├─ Presentation:
│   │   │   ├─ Facial paralysis: LMN, same as Bell's
│   │   │   ├─ Vesicles: Painful vesicular rash in ear canal, concha, soft palate
│   │   │   ├─ Severe otalgia: Ear pain (often precedes rash)
│   │   │   ├─ Hearing loss: Sensorineural (CN VIII involvement)
│   │   │   ├─ Vertigo: Vestibular involvement (CN VIII)
│   │   │   └─ Taste loss, hyperacusis (same as Bell's)
│   │   │
│   │   ├─ Diagnosis:
│   │   │   ├─ Clinical: Vesicles + facial paralysis = Ramsay Hunt
│   │   │   ├─ Zoster sine herpete: Facial paralysis without vesicles (10-20%)
│   │   │   └─ VZV serology: If no vesicles, can check VZV IgM/IgG
│   │   │
│   │   ├─ Treatment:
│   │   │   ├─ Corticosteroids: Same as Bell's palsy (prednisone 60-80 mg/day)
│   │   │   ├─ Antivirals (ESSENTIAL!): Acyclovir 800 mg 5x/day OR Valacyclovir 1000 mg 3x/day x 7-10 days
│   │   │   ├─ Eye protection: Same as Bell's palsy
│   │   │   └─ Pain management: Gabapentin, pregabalin for neuropathic pain
│   │   │
│   │   └─ Prognosis:
│   │       ├─ Worse than Bell's palsy: Complete recovery only 50-60%
│   │       ├─ Residual weakness: Common (30-40%)
│   │       └─ Postherpetic neuralgia: Chronic pain (10-20%)
│   │
│   ├─ Hemifacial Spasm
│   │   ├─ Definition: Involuntary, intermittent twitching/contractions of facial muscles
│   │   ├─ Laterality: UNILATERAL (almost always)
│   │   ├─ Age: Middle-aged to elderly, women > men
│   │   │
│   │   ├─ Etiology:
│   │   │   ├─ Primary (90%):
│   │   │   │   ├─ Vascular compression: AICA or PICA loop compresses CN VII at REZ (root exit zone)
│   │   │   │   ├─ Mechanism: Pulsatile compression → demyelination → ephaptic transmission
│   │   │   │   ├─ Ephaptic transmission: Impulses "jump" between demyelinated axons
│   │   │   │   └─ Result: Synchronous firing of multiple facial muscles
│   │   │   │
│   │   │   └─ Secondary (10%):
│   │   │       ├─ CPA tumor: Acoustic neuroma, meningioma, epidermoid
│   │   │       ├─ Multiple sclerosis: Demyelinating plaque
│   │   │       ├─ Post-Bell's palsy: Aberrant regeneration
│   │   │       └─ Brainstem lesion: Glioma, vascular malformation
│   │   │
│   │   ├─ Presentation:
│   │   │   ├─ Onset: Gradual, progressive over years
│   │   │   ├─ Twitching pattern (typical progression):
│   │   │   │   ├─ Starts: Orbicularis oculi (eye twitching)
│   │   │   │   ├─ Spreads: Downward to lower face (mouth, chin)
│   │   │   │   └─ Severity: Mild twitching → sustained contractions
│   │   │   │
│   │   │   ├─ Triggers: Stress, fatigue, voluntary facial movement
│   │   │   ├─ Persists: During sleep (unlike psychogenic)
│   │   │   └─ No pain: Painless (unlike trigeminal neuralgia)
│   │   │
│   │   ├─ Workup:
│   │   │   ├─ MRI brain: Rule out secondary causes (tumor, MS, vascular lesion)
│   │   │   ├─ MRI with FIESTA/CISS: High-resolution to visualize vascular compression
│   │   │   └─ EMG: Lateral spread response (abnormal muscle recruitment)
│   │   │
│   │   ├─ Treatment:
│   │   │   ├─ Botulinum Toxin (BoNT) - First Line:
│   │   │   │   ├─ Injection: Into affected muscles (orbicularis oculi, others)
│   │   │   │   ├─ Mechanism: Blocks acetylcholine release → muscle relaxation
│   │   │   │   ├─ Duration: 3-6 months (repeat injections needed)
│   │   │   │   ├─ Efficacy: 85-95% improvement
│   │   │   │   └─ Side effects: Ptosis, facial weakness (temporary)
│   │   │   │
│   │   │   ├─ Medications (Limited Efficacy):
│   │   │   │   ├─ Carbamazepine: Modest benefit (20-30%)
│   │   │   │   ├─ Baclofen: Muscle relaxant, limited benefit
│   │   │   │   └─ Not curative: Symptoms return when stopped
│   │   │   │
│   │   │   └─ Microvascular Decompression (MVD) - Definitive:
│   │   │       ├─ Indication: Medically refractory, desire for cure
│   │   │       ├─ Approach: Retrosigmoid craniotomy (lateral suboccipital)
│   │   │       ├─ Technique:
│   │   │       │   ├─ Position: Lateral or semi-sitting
│   │   │       │   ├─ Craniotomy: Retrosigmoid, open CPA cistern
│   │   │       │   ├─ Identify: CN VII at brainstem exit (REZ)
│   │   │       │   ├─ Visualize: Compressing vessel (AICA, PICA, vertebral artery)
│   │   │       │   ├─ Interpose: Teflon felt pledgets between vessel and nerve
│   │   │       │   └─ Secure: Teflon in place (no contact between vessel and nerve)
│   │   │       │
│   │   │       ├─ Intraoperative monitoring:
│   │   │       │   ├─ Facial EMG: Detect nerve irritation
│   │   │       │   ├─ Brainstem auditory evoked potentials (BAEP): Monitor CN VIII
│   │   │       │   └─ Lateral spread response: Should disappear if decompression successful
│   │   │       │
│   │   │       ├─ Success rate:
│   │   │       │   ├─ Complete cure: 85-90%
│   │   │       │   ├─ Significant improvement: 95%
│   │   │       │   └─ Recurrence: 5-10% (often amenable to BoNT)
│   │   │       │
│   │   │       ├─ Complications:
│   │   │       │   ├─ Hearing loss: 3-10% (CN VIII injury)
│   │   │       │   ├─ Facial weakness: 5-10% (usually temporary)
│   │   │       │   ├─ CSF leak: 5%
│   │   │       │   ├─ Meningitis: 1-2%
│   │   │       │   └─ Cerebellar injury: Rare (<1%)
│   │   │       │
│   │   │       └─ Recovery:
│   │   │           ├─ Immediate: Some patients (spasms stop in OR)
│   │   │           ├─ Gradual: Most patients (3-6 months)
│   │   │           └─ Delayed: Rare (up to 1 year for full improvement)
│   │   │
│   │   └─ Differential Diagnosis:
│   │       ├─ Blepharospasm: BILATERAL eyelid spasms (vs unilateral hemifacial spasm)
│   │       ├─ Myokymia: Benign eyelid twitching (orbicularis oculi only, doesn't spread)
│   │       ├─ Tic disorder: Voluntary suppression possible (hemifacial spasm involuntary)
│   │       └─ Psychogenic: Atypical pattern, doesn't persist in sleep
│   │
│   └─ Facial Nerve Grading (House-Brackmann Scale)
│       ├─ Purpose: Standardized assessment of facial nerve function
│       ├─ Used: Pre/postop documentation, prognostication, research
│       │
│       ├─ Grade I - Normal:
│       │   ├─ Facial symmetry and tone normal
│       │   └─ All areas of face move normally
│       │
│       ├─ Grade II - Mild Dysfunction:
│       │   ├─ Slight weakness on close inspection
│       │   ├─ Synkinesis barely noticeable
│       │   └─ At rest: Normal symmetry and tone
│       │
│       ├─ Grade III - Moderate Dysfunction:
│       │   ├─ Obvious difference between two sides, but not disfiguring
│       │   ├─ Complete eye closure with effort
│       │   ├─ Forehead: Slight to moderate movement
│       │   ├─ Mouth: Slight asymmetry with movement
│       │   └─ At rest: Normal symmetry and tone
│       │
│       ├─ Grade IV - Moderately Severe Dysfunction:
│       │   ├─ Obvious weakness and/or disfiguring asymmetry
│       │   ├─ Incomplete eye closure
│       │   ├─ Forehead: None to slight movement
│       │   ├─ Mouth: Asymmetric with maximum effort
│       │   └─ At rest: Normal symmetry and tone
│       │
│       ├─ Grade V - Severe Dysfunction:
│       │   ├─ Barely perceptible motion
│       │   ├─ Incomplete eye closure
│       │   ├─ Forehead: No movement
│       │   ├─ Mouth: Slight movement
│       │   └─ At rest: Asymmetric
│       │
│       └─ Grade VI - Total Paralysis:
│           ├─ No movement
│           ├─ Loss of tone
│           └─ No synkinesis, contracture, or spasm
│
└─ Surgical Considerations (Major Operations)
    │
    ├─ Parotidectomy (Parotid Gland Surgery)
    │   │
    │   ├─ Indications: Parotid tumor (benign or malignant), chronic parotitis, sialolithiasis
    │   │
    │   ├─ Anatomy (Key Points):
    │   │   ├─ CN VII enters: Posterior surface of parotid (after stylomastoid foramen)
    │   │   ├─ Divides: Within parotid into terminal branches
    │   │   ├─ Plane: Nerve is superficial to retromandibular vein and external carotid artery
    │   │   └─ Superficial vs deep lobe: Nerve defines plane (superficial lobe = lateral to nerve)
    │   │
    │   ├─ Approaches:
    │   │   ├─ Modified Blair incision: Preauricular → posterior auricular → neck
    │   │   ├─ Facelift incision: Cosmetically superior (hidden in hairline)
    │   │   └─ Partial vs total parotidectomy: Depends on tumor location
    │   │
    │   ├─ Technique (Nerve Identification & Preservation):
    │   │   ├─ Landmarks for CN VII trunk:
    │   │   │   ├─ Tragal pointer: Cartilaginous pointer at root of helix → nerve 1 cm deep and inferior
    │   │   │   ├─ Tympanomastoid suture: Nerve exits 6-8 mm deep and anterior to suture
    │   │   │   ├─ Posterior belly of digastric: Follow superiorly → nerve exits just above
    │   │   │   └─ Styloid process: Nerve lateral to styloid (retract styloid medially)
    │   │   │
    │   │   ├─ Nerve dissection:
    │   │   │   ├─ Identify: Main trunk at stylomastoid foramen
    │   │   │   ├─ Trace: Anteriorly into parotid (divides into terminal branches)
    │   │   │   ├─ Dissect: Parotid tissue off nerve (blunt dissection, careful hemostasis)
    │   │   │   └─ Preserve: All branches if benign tumor
    │   │   │
    │   │   ├─ Intraoperative nerve monitoring (IONM):
    │   │   │   ├─ Facial EMG electrodes: Frontalis, orbicularis oculi, orbicularis oris, mentalis
    │   │   │   ├─ Nerve stimulator: 0.5-1.0 mA threshold
    │   │   │   ├─ Alerts: If nerve stimulated (helps identify branches)
    │   │   │   └─ Efficacy: Does NOT prevent injury, but aids identification
    │   │   │
    │   │   └─ Tumor removal:
    │   │       ├─ Benign (pleomorphic adenoma): Preserve all branches
    │   │       ├─ Malignant: Sacrifice nerve if involved (plan reconstruction)
    │   │       └─ Margin: Adequate clearance (malignant) vs nerve preservation (benign)
    │   │
    │   ├─ Key Branches at Risk:
    │   │   ├─ Temporal branch: Crosses zygomatic arch (superficial) → injury = forehead paralysis
    │   │   ├─ Marginal mandibular: Runs below mandible → injury = asymmetric smile
    │   │   └─ Identification: Use IONM to confirm branch identity before dissection
    │   │
    │   ├─ Complications:
    │   │   ├─ Temporary facial weakness: 20-40% (neuropraxia, resolves weeks-months)
    │   │   ├─ Permanent facial paralysis: 1-5% (benign tumors), 20-40% (malignant if nerve sacrificed)
    │   │   ├─ Frey syndrome (gustatory sweating): 30-50%
    │   │   │   ├─ Mechanism: Parasympathetic fibers regenerate to sweat glands
    │   │   │   ├─ Presentation: Sweating/flushing over parotid area when eating
    │   │   │   └─ Treatment: BoNT injections, topical antiperspirant
    │   │   │
    │   │   ├─ Greater auricular nerve: Sensory loss to earlobe (often unavoidable)
    │   │   ├─ Sialocele: Saliva collection (drain, pressure dressing)
    │   │   └─ Hematoma: Drain if significant (risk airway compromise)
    │   │
    │   └─ Postoperative Care:
    │       ├─ Monitor: Facial function immediately postop (document House-Brackmann grade)
    │       ├─ Eye protection: If lagophthalmos (artificial tears, taping at night)
    │       ├─ Drain: Usually placed, remove when output <30 mL/day
    │       └─ Follow-up: Regular assessment of facial function, tumor recurrence
    │
    ├─ Acoustic Neuroma (Vestibular Schwannoma) Surgery
    │   │
    │   ├─ Tumor: Benign schwannoma arising from CN VIII (superior or inferior vestibular nerve)
    │   ├─ Location: CPA and/or IAC
    │   │
    │   ├─ Surgical Approaches:
    │   │   ├─ Translabyrinthine:
    │   │   │   ├─ Indication: Large tumor, no useful hearing
    │   │   │   ├─ Advantage: Direct access to IAC, early identification of CN VII
    │   │   │   ├─ Disadvantage: Sacrifices hearing
    │   │   │   └─ CN VII: Identified early at IAC fundus, preserved throughout
    │   │   │
    │   │   ├─ Retrosigmoid (Lateral Suboccipital):
    │   │   │   ├─ Indication: Any size tumor, attempt hearing preservation
    │   │   │   ├─ Advantage: Hearing preservation possible (small tumors)
    │   │   │   ├─ Disadvantage: Cerebellar retraction, CN VII identified late
    │   │   │   └─ CN VII: Usually on anterior surface of tumor (identify after debulking)
    │   │   │
    │   │   └─ Middle Fossa:
    │   │       ├─ Indication: Small intracanalicular tumor (<1.5 cm), good hearing
    │   │       ├─ Advantage: Excellent hearing preservation, direct access to IAC
    │   │       ├─ Disadvantage: Temporal lobe retraction, limited CPA exposure
    │   │       └─ CN VII: Identified at IAC, dissected anteriorly
    │   │
    │   ├─ Technique (CN VII Preservation):
    │   │   ├─ Nerve location: Usually anterior or anterosuperior surface of tumor
    │   │   ├─ Identification:
    │   │   │   ├─ Early (translabyrinthine): At IAC fundus (Bill's bar separates VII from VIII)
    │   │   │   ├─ Late (retrosigmoid): After tumor debulking, identify on anterior capsule
    │   │   │   └─ Dissection plane: Between tumor and nerve (arachnoid plane)
    │   │   │
    │   │   ├─ Tumor removal:
    │   │   │   ├─ Debulk: Internal decompression (ultrasonic aspirator, suction)
    │   │   │   ├─ Capsule: Dissect off CN VII (sharp dissection, preserve arachnoid)
    │   │   │   ├─ Gentle: Minimize manipulation (nerve very sensitive to traction)
    │   │   │   └─ Goal: Gross total resection if safe, subtotal if nerve at risk
    │   │   │
    │   │   ├─ Intraoperative monitoring (CRITICAL!):
    │   │   │   ├─ Facial EMG: Continuous monitoring, nerve stimulator
    │   │   │   ├─ Evoked potentials: Direct nerve stimulation (confirm integrity)
    │   │   │   ├─ Train-of-four: Repetitive stimulation (assess nerve fatigue)
    │   │   │   └─ Alarm criteria: Loss of response = high risk injury
    │   │   │
    │   │   └─ Decision-making:
    │   │       ├─ If nerve anatomically intact: Good chance of facial function recovery
    │   │       ├─ If nerve thinned but intact: May have delayed recovery (months)
    │   │       └─ If subtotal resection: Residual tumor preferred over nerve sacrifice (benign tumor)
    │   │
    │   ├─ Facial Nerve Outcomes:
    │   │   ├─ Anatomic preservation: >95% (modern series)
    │   │   ├─ Good facial function (HB I-II):
    │   │   │   ├─ Small tumors (<1.5 cm): 90-95%
    │   │   │   ├─ Medium tumors (1.5-3 cm): 70-80%
    │   │   │   └─ Large tumors (>3 cm): 50-60%
    │   │   │
    │   │   ├─ Immediate postop: Often HB III-IV (neuropraxia)
    │   │   ├─ Recovery: Gradual over 6-12 months (nerve stretched but not cut)
    │   │   └─ If HB V-VI at 1 year: Consider facial reanimation
    │   │
    │   ├─ Complications:
    │   │   ├─ Facial weakness: See above
    │   │   ├─ Hearing loss: Depends on approach and tumor size
    │   │   ├─ CSF leak: 5-10% (manage with lumbar drain, rarely needs repair)
    │   │   ├─ Meningitis: 1-2%
    │   │   └─ Cerebellar injury: Rare (<1%)
    │   │
    │   └─ Alternative: Stereotactic Radiosurgery (Gamma Knife, CyberKnife)
    │       ├─ Indication: Small-medium tumors, elderly, medical comorbidities, patient preference
    │       ├─ Facial nerve preservation: 95-98% (better than surgery for some tumors)
    │       ├─ Hearing preservation: 50-70% (small tumors with good hearing)
    │       └─ Limitations: Does not remove tumor (tumor control, not cure), delayed effect
    │
    └─ Facial Reanimation Procedures
        │
        ├─ Indication: Permanent complete facial paralysis (HB V-VI at 1+ year)
        │
        ├─ Timing Considerations:
        │   ├─ Acute (<72 hours): Primary neurorrhaphy if nerve transected
        │   ├─ Subacute (3 days - 2 years): Nerve grafting, nerve transfer
        │   ├─ Chronic (>2 years): Muscle denervated → only muscle transfer/free flap
        │   └─ Key: Muscle atrophy occurs ~2 years after denervation (window for reinnervation)
        │
        ├─ Procedures (Spectrum of Options):
        │   │
        │   ├─ 1. Primary Neurorrhaphy (Direct Repair):
        │   │   ├─ Indication: Immediate recognition of nerve transection (iatrogenic, trauma)
        │   │   ├─ Timing: Best if within 72 hours (up to 7 days acceptable)
        │   │   ├─ Technique:
        │   │   │   ├─ Identify: Both nerve ends
        │   │   │   ├─ Debride: Back to healthy fascicles (remove damaged tissue)
        │   │   │   ├─ Approximate: Tension-free alignment (9-0 or 10-0 nylon sutures)
        │   │   │   └─ Epineural: Suture epineurium (NOT individual fascicles - too small)
        │   │   │
        │   │   └─ Prognosis: Best outcome if primary repair (HB II-III in 70-80%)
        │   │
        │   ├─ 2. Interpositional Nerve Graft (Cable Graft):
        │   │   ├─ Indication: Gap too large for primary repair (>1-2 cm)
        │   │   ├─ Timing: Up to 2 years post-injury (before muscle atrophy)
        │   │   ├─ Donor nerves:
        │   │   │   ├─ Great auricular nerve (most common): Easy access, adequate length
        │   │   │   ├─ Sural nerve: If longer graft needed (harvest from ankle)
        │   │   │   └─ Sacrifice: Sensory loss in donor area (acceptable trade-off)
        │   │   │
        │   │   ├─ Technique:
        │   │   │   ├─ Harvest: Donor nerve (adequate length)
        │   │   │   ├─ Interpose: Between proximal and distal CN VII stumps
        │   │   │   ├─ Suture: 9-0 nylon, tension-free (proximal and distal anastomoses)
        │   │   │   └─ Goal: Bridge gap, allow axonal regeneration
        │   │   │
        │   │   └─ Prognosis: HB III-IV in 50-70% (worse than primary repair)
        │   │
        │   ├─ 3. Nerve Transfer (Cross-Facial Nerve Graft + Masseteric Nerve):
        │   │   ├─ Indication: Proximal nerve injury (no viable CN VII proximal stump)
        │   │   ├─ Timing: Up to 2 years post-injury
        │   │   │
        │   │   ├─ Cross-facial nerve graft (CFNG):
        │   │   │   ├─ Principle: Use contralateral CN VII branches to power paralyzed side
        │   │   │   ├─ Technique:
        │   │   │   │   ├─ Identify: Buccal branches on normal side (identify with stimulator)
        │   │   │   │   ├─ Divide: 30-50% of buccal branches (avoid weakening donor side)
        │   │   │   │   ├─ Graft: Sural nerve from donor side across face to paralyzed side
        │   │   │   │   ├─ Coaptation: Distal graft to facial nerve branches on paralyzed side
        │   │   │   │   └─ Wait: 9-12 months for axons to grow across face
        │   │   │   │
        │   │   │   ├─ Advantage: Spontaneous, symmetric smiling (mirror movements)
        │   │   │   └─ Disadvantage: Long delay (12+ months), weak smile
        │   │   │
        │   │   └─ Masseteric nerve transfer (alternative/adjunct):
        │   │       ├─ Principle: Masseteric nerve (CN V3 branch) transferred to distal CN VII
        │   │       ├─ Technique:
        │   │       │   ├─ Identify: Masseteric nerve (deep to masseter muscle)
        │   │       │   ├─ Divide: Distal masseteric nerve
        │   │       │   ├─ Transfer: Coaptation to buccal branches or main CN VII trunk
        │   │       │   └─ Reinnervation: 3-6 months (faster than CFNG!)
        │   │       │
        │   │       ├─ Advantage: Faster recovery, stronger contraction
        │   │       ├─ Disadvantage: Non-spontaneous (clench teeth → smile), not symmetric
        │   │       └─ Combined: CFNG + masseteric (best of both)
        │   │
        │   ├─ 4. Regional Muscle Transfer:
        │   │   ├─ Indication: Chronic paralysis (>2 years), muscle atrophy
        │   │   ├─ Principle: Transfer nearby innervated muscle to restore smile
        │   │   │
        │   │   ├─ Temporalis muscle transfer:
        │   │   │   ├─ Technique: Rotate temporalis muscle inferiorly, attach to oral commissure
        │   │   │   ├─ Reanimation: Clench teeth → corner of mouth elevates
        │   │   │   └─ Advantage: Strong, durable
        │   │   │
        │   │   └─ Masseter muscle transfer:
        │   │       ├─ Similar to temporalis, rotate masseter to oral commissure
        │   │       └─ Advantage: Less temporal hollowing than temporalis
        │   │
        │   └─ 5. Free Functional Muscle Transfer:
        │       ├─ Indication: Chronic paralysis (>2 years), best cosmetic result
        │       ├─ Principle: Transfer innervated muscle with vascular pedicle (microvascular)
        │       │
        │       ├─ Gracilis muscle (most common):
        │       │   ├─ Harvest: From medial thigh (with obturator nerve and vascular pedicle)
        │       │   ├─ Transfer: To face (attach to zygomatic arch and oral commissure)
        │       │   ├─ Neurotization: Coaptation to masseteric nerve or CFNG
        │       │   ├─ Vascular: Anastomose to facial artery/vein (microvascular)
        │       │   └─ Reinnervation: 6-12 months for movement
        │       │
        │       ├─ Advantage: Best cosmetic result, powerful smile
        │       ├─ Disadvantage: Complex surgery (8-12 hours), microvascular expertise required
        │       └─ Success: 85-90% functional smile
        │
        ├─ Adjunctive Procedures (Static vs Dynamic):
        │   │
        │   ├─ Static Procedures (No Movement):
        │   │   ├─ Gold weight implant (upper eyelid):
        │   │   │   ├─ Indication: Lagophthalmos (cannot close eye)
        │   │   │   ├─ Technique: Implant gold weight in upper eyelid (gravity aids closure)
        │   │   │   └─ Efficacy: 90% improve eye closure (protect cornea)
        │   │   │
        │   │   ├─ Lower eyelid tightening:
        │   │   │   ├─ Lateral tarsal strip, lateral canthoplasty
        │   │   │   └─ Goal: Prevent ectropion, improve eye closure
        │   │   │
        │   │   ├─ Brow lift:
        │   │   │   ├─ Suspend drooping brow (direct, endoscopic, or temporal lift)
        │   │   │   └─ Improve: Symmetry, reduce lagophthalmos
        │   │   │
        │   │   └─ Static sling:
        │   │       ├─ Fascia lata graft from corner of mouth to zygomatic arch
        │   │       └─ Goal: Elevate oral commissure (cosmetic, not functional)
        │   │
        │   └─ Chemodenervation (Botulinum Toxin):
        │       ├─ Indication: Synkinesis (overactivity on normal side or aberrant regeneration)
        │       ├─ Technique: Inject BoNT into overactive muscles
        │       └─ Goal: Balance facial symmetry (weaken hyperactive side)
        │
        └─ Multidisciplinary Approach:
            ├─ Neurosurgery: Nerve repair, nerve grafting
            ├─ Plastic surgery: Muscle transfers, free flaps
            ├─ Oculoplastic: Eyelid procedures (gold weight, tarsal strip)
            ├─ Physical therapy: Facial neuromuscular retraining
            └─ Patient counseling: Realistic expectations, long timeline (12-24 months)
```

---